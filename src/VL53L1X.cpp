...
bool VL53L1X::begin(uint8_t deviceAddress)
{
  ...
  for (uint16_t i = 0; i < 36; i++) {
	  uint8_t regVal = cpi2c_readRegister(_i2cport, i + 1);
...
void VL53L1X::startMeasurement(uint8_t offset)
{
  ...
  while (leftToSend > 0)
  {
    ...
    cpi2c_beginTransmission(_i2cport);

    cpi2c_write(0);//We're only in lower address space. No MSB needed.
    cpi2c_write(address);

    for (uint8_t x = 0 ; x < toSend ; x++) {
      cpi2c_write(configBlock[address + x - 1 - offset]);
    }
...
{
  if (cpi2c_readRegister(_i2cport, VL53L1_GPIO__TIO_HV_STATUS) != 0x03) return(true); //New measurement!
...
void VL53L1X::setDistanceMode(uint8_t mode)
{
	...
	//timing
	cpi2c_writeRegister_16_8(_i2cport, VL53L1_RANGE_CONFIG__VCSEL_PERIOD_A, periodA);
	cpi2c_writeRegister_16_8(_i2cport, VL53L1_RANGE_CONFIG__VCSEL_PERIOD_B , periodB);
	cpi2c_writeRegister_16_8(_i2cport, VL53L1_RANGE_CONFIG__VALID_PHASE_HIGH, phaseHigh);
	
	//dynamic
	cpi2c_writeRegister_16_8(_i2cport, VL53L1_SD_CONFIG__WOI_SD0 , periodA);
	cpi2c_writeRegister_16_8(_i2cport, VL53L1_SD_CONFIG__WOI_SD1, periodB);
	cpi2c_writeRegister_16_8(_i2cport, VL53L1_SD_CONFIG__INITIAL_PHASE_SD0, phaseInit);
	cpi2c_writeRegister_16_8(_i2cport, VL53L1_SD_CONFIG__INITIAL_PHASE_SD1, phaseInit);
...
#define VL53L1_RANGESTATUS_NONE        255 /*!<No Update. */
  ...
  //Read status
  uint8_t measurementStatus = cpi2c_readRegister(_i2cport, VL53L1_RESULT__RANGE_STATUS) & 0x1F;
